...
import { GridRow, GridCol, Modal, ModalDrawer, GridContainer } from '@taikai/rocket-kit';
...
})(ConnectorsNames || (ConnectorsNames = {}));
...
function ConnectorButton({ activeChainId, setError, connector, isActive, error, onConnectorConnect, logo, onConnectorDisconnect, variant }) {
    ...
    const switchChain = useCallback(async (_desiredChainId) => {
        var _a;
        try {
    ...
    return jsx("div", Object.assign({ style: { margin: "1rem 0 0", display: "inline-flex", alignItems: "center" } }, { children: isActive
            ? error
                ? (jsx(Action, { variant: variant, onClick: () => retry(), children: "try again?" }))
                : (jsx(Action, { variant: variant, onClick: () => onDisconnectClick(), children: "Disconnect" }))
            : (jsx(Action, { variant: variant, onClick: () => onSelectChain(1), children: error ? "try again?" : jsxs(Fragment, { children: [logo, " ", getConnectorName(connector) || "Unnamed Implementation"] }) })) }));
}
...
const [coinbaseWallet, hooks$3] = initializeConnector((actions) => new CoinbaseWallet({ actions, options: { url: Chains[0].urls[0], appName: process.env.APP_NAME || "@taikai/dappkit" } }));
...
const useDappkit = create((set, get) => ({
...
const useDappkitConnection = () => useDappkit(useShallow(({ connection }) => ({ connection })));
...
function CoinbaseButton({ onConnectorConnect, onConnectorDisconnect, variant }) {
    ...
    return jsx(ConnectorButton, { activeChainId: chainId || 0, logo: jsx(Logo$3, {}), variant: variant, setError: setError, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect, connector: coinbaseWallet, isActive: connected && isActive, error: error });
}

const [metamaskWallet, hooks$2] = initializeConnector((actions) => new MetaMask({ actions }));
...
function MetamaskButton({ onConnectorConnect, onConnectorDisconnect, variant }) {
    ...
    const { chainId, connected } = useDappkitConnectionInfo();
    return jsx(ConnectorButton, { connector: metamaskWallet, logo: jsx(Logo$2, {}), variant: variant, activeChainId: chainId || 0, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect, isActive: connected && isActive, setError: setError, error: error });
}

const [gnosisSafe, hooks$1] = initializeConnector((actions) => new GnosisSafe({ actions }));
...
function GnosisSafeButton({ onConnectorConnect, onConnectorDisconnect, variant }) {
    ...
    const { chainId, connected } = useDappkitConnectionInfo();
    return jsx(ConnectorButton, { connector: gnosisSafe, logo: jsx(Logo$1, {}), variant: variant, activeChainId: chainId || 0, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect, isActive: connected && isActive, setError: setError, error: error });
}

const WALLET_CONNECT_PROJECT_ID = process.env.WALLET_CONNECT_PROJECT_ID;
const [walletConnect, hooks] = initializeConnector(actions => new WalletConnect({
    actions,
...
function WalletConnectButton({ onConnectorConnect, onConnectorDisconnect, variant }) {
    ...
    const { chainId, connected } = useDappkitConnectionInfo();
    return jsx(ConnectorButton, { connector: walletConnect, logo: jsx(Logo, {}), variant: variant, activeChainId: chainId || 0, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect, isActive: connected && isActive, setError: setError, error: error });
}

const GlobalStyles = createGlobalStyle `html {
    --vh: 10px !important;
  }`;
const GridRowFlexWrap = styled(GridRow) `
    flex-wrap: wrap;
    ${(props) => props.variant === ModalModes.Modal ? "gap: 10px" : ""}
  `;
const GridColNoGrowVariant = styled(GridCol) `
...
function WalletSelector({ showWallets, showModal, modalCloseClicked = (() => {
    ...
    function renderModalChildren() {
        return jsxs(GridContainer, { children: [jsx(GridRow, { children: jsx(GridCol, Object.assign({ className: "wallet-connected-address" }, { children: address })) }), !showWallets.length ? jsx(GridRow, { children: jsx(GridCol, { children: "No allowed list provided" }) }) : null, jsxs(GridRowFlexWrap, Object.assign({ variant: mode }, { children: [showWallets.includes(ConnectorsNames.Coinbase) ?
                            jsx(GridColNoGrowVariant, { variant: mode, children: jsx(CoinbaseButton, { variant: mode, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect }) }) : null, showWallets.includes(ConnectorsNames.Metamask) ?
                            jsx(GridColNoGrowVariant, { variant: mode, children: jsx(MetamaskButton, { variant: mode, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect }) }) : null, showWallets.includes(ConnectorsNames.GnosisSafe) ?
                            jsx(GridColNoGrowVariant, { variant: mode, children: jsx(GnosisSafeButton, { variant: mode, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect }) }) : null, showWallets.includes(ConnectorsNames.WalletConnect) ?
                            jsx(GridColNoGrowVariant, { variant: mode, children: jsx(WalletConnectButton, { variant: mode, onConnectorConnect: onConnectorConnect, onConnectorDisconnect: onConnectorDisconnect }) }) : null] }))] });
    }
    return jsxs(Fragment, { children: [jsx(GlobalStyles, {}), jsx("div", Object.assign({ className: "wallet-selector-container" }, { children: mode === ModalModes.Modal
                    ? jsx(Modal, { isShowing: showModal, hide: modalCloseClicked, title: modalTitle, children: renderModalChildren(), footer: false })
                    : jsx(ModalDrawer, { isShowing: showModal, hide: modalCloseClicked, title: modalTitle, children: renderModalChildren() }) }))] });
}

export { CoinbaseButton, ConnectorButton, ConnectorsNames, GnosisSafeButton, MetamaskButton, WalletSelector, useDappkit, useDappkitConnection, useDappkitConnectionInfo };
